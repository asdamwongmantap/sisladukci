		  ...
		  <!--list usergroup-->
			<div class="row">
              <div class="col-md-12 col-sm-4 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>List Data Surat Kuasa</h2>
                    <div class="clearfix"></div>
					
                  </div>
                  <div class="x_content">
				  <input type="hidden" id="usergroup" value="<?=$this->session->userdata('usergroupid');?>">
				  <a href="<?=base_url('app/surat/add_suratkuasa');?>" class="btn btn-success" title="Tambah user group" data-target=".bs-example-modal-smadd" style="float:right;display:block;" 
				  id="tomboltambah"><i class="fa fa-plus"></i> Buat Surat Kuasa</a></br>
				  </br>
				  <table id="mydata" class="table table-striped table-bordered nowrap" cellspacing="0" width="100%">
                      ...
                      <tbody id="show_data">
							...
							<?php 
							foreach ($datakuasa as $row) {?>	 
							<tr>
									<td><?=$row->no_surat;?></td>							
									<td><?=$row->wrg_nama;?></td>
									<td><?=$row->wrg_kwarganegaraan;?></td>
									<td><?=$row->tgl_surat;?></td>							
									<td><a class="btn btn-success" href='pdfkuasa/<?=$row->no_surat;?>'><i class="glyphicon glyphicon-zoom-in icon-white"></i></a>
