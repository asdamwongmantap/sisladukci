		  ...
		  <!--list usergroup-->
			<div class="row">
              <div class="col-md-12 col-sm-4 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Data Kematian Penduduk</h2>
                    <div class="clearfix"></div>
					
                  </div>
                  <div class="x_content">
				  <input type="hidden" id="usergroup" value="<?=$this->session->userdata('usergroupid');?>">
				  <a href="<?=base_url('app/data/warga/addmeninggalwarga');?>" class="btn btn-success" title="Tambah user group" data-target=".bs-example-modal-smadd" style="float:right;display:block;" 
				  ...
				  <table id="mydata" class="table table-striped table-bordered nowrap" cellspacing="0" width="100%">
                      ...
                      <tbody id="show_data">
							...
							<?php 
							$no = 1;
							foreach ($datameninggalwarga as $row) {?>	 
							<tr>
									<td><?=$no;?></td>
									<td><a href='detailwarga/<?=$row->wrgmeninggal_nik;?>'><?=$row->wrgmeninggal_nik;?></a></td>							
									<td><?=$row->wrg_nama;?></td>
